...
灯具布局仍由创作者决定，放在实心柱下的灯可能被遮挡，预检会指出实际暗区。
辅助方法不改变 KEEP 语义，也不暗中选择建筑风格、群组成员或世界位置。

绘图与语义侧车都冻结保存，在格式 3 世界包的结构模块模式 2 中保存绘图引用和语义 JSON。
...
DrawProgram 和 JSON 蓝图创作路线保留；世界包现统一为格式 3，旧的未发布格式 1/2 需要重新生成。没有扩展结构世界生成范围、采样或写入预算，
