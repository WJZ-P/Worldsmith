# Custom creature runtime, schemas 1 and 2
...
- The natural/creative hosts, `worldsmith:content/creature/passive` and `worldsmith:content/creature/hostile`, use vanilla CREATURE and MONSTER categories. A third, `worldsmith:content/creature/encounter_boss`, serves typed landmark spawners only. Hostile hosts implement `Enemy` and are disallowed in peaceful difficulty; ordinary guards do not consume the separate spawner class's proximity slots.
- World-local definitions configure health, movement speed, follow distance, attack damage, knockback resistance and collision width/height. Passive creatures remain when players walk away; ordinary hostile creatures use vanilla mob despawning. Explicit Bosses suppress distance despawn without claiming world uniqueness.
...
`CreatureLibrary(schemaVersion=1, creatures=[...])` contains ordinary definitions with model, attributes, behavior, spawn, themeRole and optional drops. A non-null `CreatureDefinition.boss` requires creature schema 2 and bundle format 5; schema-1 serialization omits that field.
...
- Core schema-1 health remains 1–2048 for legacy identity, but current native preparation explicitly rejects values above 1024 instead of clamping; new playable definitions and Boss profiles use health <=1024. Speed is 0.01–1, follow range 4–64, damage 0–100, knockback resistance 0–1, collision width 0.2–4 and height 0.2–6 blocks. Native preparation also checks every base attribute and Boss-derived speed/damage against the actual game attributes.
...
- `prepared.serverResources()` includes native block loot/tool tags **and the complete world-content bundle** under `worldsmith-content/`, with exact native slots stored separately at `worldsmith-runtime/block-bindings.json`. New authored bundles use format 5 with nine modules; legacy formats 3/4 retain their original identity during read-only restoration and unchanged re-embedding. Export these into the generated datapack that travels with the save; reopening must not depend on an author's config directory.
...
DSL or arbitrary condition script. Drops themselves do not grant quest progress;
the separate quest runtime validates its kill/delivery objectives. Empty drops are omitted from
