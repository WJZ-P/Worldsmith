...
| 内容 | 存放方式 |
| --- | --- |
| 内容清单、依赖和资源摘要 | `worldsmith.json`，当前世界格式 10 |
| 世界观、地形、群系、地物 | 对应模块 JSON |
| 方块、物品、生物、Boss、任务 | 对应模块 JSON；生物模型骨骼、立方体、UV、行为参数也在定义内 |
| 持久故事定义 | story/story.json：事实、真实地点和角色、对话、知识、交易、日程、环境音；玩家状态留在存档 |
| 建筑与组装 | 结构索引、蓝图和 `drawings/<hash>.wsdraw` 冻结几何 |
| PNG 原始字节 | `assets/<sha256>.png`，同一资产只列一份 |
| 作者源码出处 | 既有结构来源记录，作为可读数据保留，不在导入或运行世界时编译执行 |

旧格式 1–9 被拒绝导入；现有本地文件不自动迁移或改写。格式 10 必含全部十二模块，包括 mechanics.json、abilities/abilities.json、story/story.json，即使其库为空；quests 使用 schema2。
...
4. 外部归档在“用于创建世界”时先完成导入完整性检查；已安装包只读取选择信息并打开设置页。两者均不在此时加载世界资源，真正准备延迟到原版“创建新的世界”按钮。
...
入场提示持续 5 秒。已准备完成的天工开物世界包在创建时跳过通用的“实验性设置”确认；
重新进入存档时，仅在主世界生成器身份与已校验的内嵌世界包一致时跳过对应确认。
此行为不修改存档或注册表的 lifecycle，也不重写既有世界包。实验性功能开关、旧自定义世界、
版本升级/降级备份、存档恢复和数据包错误仍走原版流程。

用世界包新建世界时，存档名默认使用世界包名称；未手动改名时，切换世界包会同步更新名称。
玩家自定义的名称保留，切回原版世界类型时只还原自动填入的名称。名称输入框和原版存档目录提示
同步更新，同名目录的避让及特殊字符处理继续使用原版逻辑，不重命名已有存档。

“世界”页的世界类型现在同时列出全部已安装包，天工开物顶部选择器与之同步；无需先分别加载每个包来注册菜单项。旧Wasteland已从新建菜单隐藏，旧存档所需定义保留。菜单浏览不改写现有世界包或存档。
