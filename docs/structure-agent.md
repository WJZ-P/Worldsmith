...
普通建筑默认使用 `READABLE` 并布置真实灯具，`lighting.spaces` 描述所有楼层、楼梯和可用阁楼。
默认发布仅校验声明、源方块与边界，不进行逐点亮度估算；`minimum` 只作可选诊断目标。
`worldsmith_preflight_structure(..., estimateLighting:true)` 或显式 lighting 叠加层可查看估算，暗区只给警告。
故意幽暗的墓室等使用 `INTENTIONALLY_DARK`；SDK 调用 `intentionallyDark(reason)` 记录创作意图。
存在室内声明时 `EXTERIOR_ONLY` 会报错；需要真实发光方块，原生检查核对其状态发光等级。
未声明的室内意图不会由校验器自动推断。Core 的光传播估算不代替 MC 光照或视觉验收。
吊灯优先使用 `hangingLightFixture(id,at,anchor)`，锚点取自真实屋梁/曲面屋顶；不要使用统一猜测高度截断铁链。
无用途的封闭屋顶夹层应填实或打开，保留为阁楼时照常布灯；这些是后续生成规则，不扫描修改旧存档。
...
新世界包采用格式 5 的九模块，其中结构模块模式 2 保存调色板/游程压缩冻结数据、源码来源、目标 MC 数据版本及工具版本，均参与内容哈希。格式 3/4 仅按原身份读取恢复或重新封装，不自动升级。
