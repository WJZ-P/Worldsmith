...
final class WorldsmithStructureFixtures {
    static WorldsmithTemplateStructure.Settings settings(Identifier template,BlockPos size,BlockPos origin,List<Rotation> rotations,
        ...
        var part=new WorldsmithStructurePlan.Part(template,offset,Rotation.NONE,size,reserved,footprint.stream().map(v->v.atY(size.getY()-1)).toList(),supports,WorldsmithInstanceProcessor.Config.EMPTY);
        ...
        var plan=new WorldsmithStructurePlan(List.of(part),columns,support,size.getY(),bounds,List.of());
        var site=new WorldsmithStructureSite(surface,-64,319,0,0,8,slope,foundation,material,depth,0,4096);
        return new WorldsmithTemplateStructure.Settings(List.of(plan),rotations,site,layout,java.util.Optional.empty());
    }
}
