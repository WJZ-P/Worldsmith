    ...
    fun tools():List<McpTool> {
        ...
        return listOf(
            ...
            McpTool("worldsmith_import_resource_pack","Import a validated single-file resource pack",
                "Import one .wspack inbox filename as the original immutable bundle. Writes reusable PNG blobs before publishing its hash directory. Existing identical content keeps its saved display metadata; only current format 10 is accepted; older formats are rejected. Does not edit a session or activate a world.",
