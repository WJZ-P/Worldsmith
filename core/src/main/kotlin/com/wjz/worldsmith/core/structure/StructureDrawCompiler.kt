...
object StructureDrawCompiler {
    ...
    fun compile(b: StructureBlueprint,drawing: DrawStructure): CompiledStructure {
        ...
        need(size.x<=193 && size.z<=193 && size.y<=128,"DRAWING_DEPLOYMENT_EXTENT","Drawing remains previewable/exportable but exceeds the current 193x128x193 deployment envelope")
        need(drawing.voxels().size<=StructureCatalogCompiler.MAX_PLAN_VOXELS,"DRAWING_DEPLOYMENT_BUDGET","Drawing exceeds the per-plan authored-cell budget including AIR; reduce authored clearance or deployment footprint")
        ...
        need(normalized.origin.x in 0 until size.x && normalized.origin.z in 0 until size.z && normalized.origin.y==0,
        ...
        need(normalized.keepClear.size<=32 && normalized.keepClear.all(::validBox),"DRAWING_CLEARANCE_BOUNDS","Invalid keepClear volume")
        val protected=b.variation.protectedAreas.map(::box)
        need(protected.size<=32 && protected.all(::validBox),"DRAWING_PROTECTED_BOUNDS","Invalid protected volume")
        ...
        val diagnostics=StructureNavigation.inspect(normalized,voxels).diagnostics+StructureContentChecks.validate(normalized,voxels.associateBy { it.position })
        diagnostics.firstOrNull { it.severity==DiagnosticSeverity.ERROR }?.let { throw StructureBuildException(it) }
        StructureLightingChecker.inspect(normalized,voxels)
        return CompiledStructure(b.id,size,normalized.origin,voxels,normalized.keepClear,voxels.size,diagnostics,normalized.interactions,
            normalized.lighting,normalized.ports,protected,min,true,anchors=drawing.anchors().mapValues { (_,v)->p(BuildPos(v.x(),v.y(),v.z())) })
    }
}
