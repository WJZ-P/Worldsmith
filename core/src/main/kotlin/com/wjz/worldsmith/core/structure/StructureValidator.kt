...
object StructureValidator {
    ...
    fun validate(library: StructureLibrary, biomes: BiomePlan): List<Diagnostic> = buildList {
        if(library.schemaVersion !in 1..3)add(error("schemaVersion","UNSUPPORTED_SCHEMA","Structure library schema must be 1, 2 or 3"))
        ...
        if(library.schemaVersion<3 && library.structures.any { s -> (listOf(s.blueprint)+s.assembly?.pieces.orEmpty().values).any { b -> b.interactions.any { it is StructureInteraction.StoryAnchor } } })
            add(error("schemaVersion","STORY_ANCHOR_SCHEMA_REQUIRED","Story anchors require structure library schema 3"))
