    ...
    @JvmStatic fun tiles(g:CompiledStructure):List<StructureTile> {
        ...
        return groups.entries.sortedWith(compareBy({it.key.y},{it.key.z},{it.key.x})).map { (key,cells)->
            ...
            StructureTile(offset,g.copy(size=size,origin=BuildPos(0,0,0),voxels=cells.map {it.copy(position=local(it.position))},
                ...
                interactions=interactions.map { (_,v)->when(v) {is StructureInteraction.Container->v.copy(at=local(v.at));is StructureInteraction.Sign->v.copy(at=local(v.at));is StructureInteraction.Banner->v.copy(at=local(v.at));is StructureInteraction.BossSpawner->v.copy(at=local(v.at));is StructureInteraction.StoryAnchor->v.copy(at=local(v.at))} },
